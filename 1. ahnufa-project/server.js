...
// Middleware harus duluan sebelum route
app.use(express.urlencoded({ extended: true }));
app.use(express.json()); // ⬅️ PENTING! HARUS SEBELUM API POST

// Import route handler
const { renderLihatFile, handleFiles } = require('./lihat-file');
const { renderDatabaseInfo } = require('./lihat-database');
const { renderNgodingAI } = require('./ngoding-ai');
const gptApi = require('./api-gpt');

// Pasang route
app.use(gptApi); // ✅ Sekarang sudah setelah express.json()

// Static dan pages
app.use(express.static('public'));

app.get('/lihat-database', renderDatabaseInfo);
app.get('/ngoding-ai', renderNgodingAI);
app.get('/lihat-file', renderLihatFile);
app.post('/files', handleFiles);
